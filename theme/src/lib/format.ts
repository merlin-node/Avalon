...
/**
 * 货币原样显示后台填的内容，大小写也不改，不做任何代码到符号的替换。
 * 以货币符号结尾的（$ ¥ € C$ HK$）放在数字前面，其余（cad、元、USD）放在后面。
 */
const ENDS_WITH_SYMBOL = /\p{Sc}$/u

export function money(amount: number, currency: string): string {
  const unit = (currency ?? "").trim()
  if (!unit) return amount.toFixed(2)
  return ENDS_WITH_SYMBOL.test(unit) ? `${unit}${amount.toFixed(2)}` : `${amount.toFixed(2)} ${unit}`
}
